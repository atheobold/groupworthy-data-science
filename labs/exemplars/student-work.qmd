...
ggplot(cond_year, aes(x = year, y = mean_K, group = species)) +
  ...
  labs(
    ...
    subtitle = "Yearly means by species with 95% CI\nLength cleaned to 5–80 cm; weight cleaned to 10–4,000 g",
    x = "Year", y = "Mean condition index (K = 100·W[g]/L[cm]^3)"
  ) +
  theme_minimal()
```
...
I thought it was awesome to see quite a few students create "helper" functions
to help make nicely formatted labels for their plot. Here are some examples!
...
While most students wrote a plotting function to make a specific type of plot 
(e.g., barplot, scatterplot), two students wrote plotting functions that 
decide what plot to make based on the data types of the variables input. So 
cool!
...
For your `style_plot()` function I asked for you to allow the user to input a
color palette to be used. But! The color palette they input might have the 
wrong number of colors! 

Amazingly, Kristen Ho figured out how to handle just this problem! Kristen's 
function inspects the `plot` object to see how many labels the plot has, and
uses that number to create a color palette with the right number of colors 
