...
# ...but loops make it difficult to understand what the code is doing

# Which is easier to understand what is being made?
...
- 450g fresh large raspberries

**For the marzipan**

- 100g ground almonds
- 100g icing sugar, plus extra for dusting
- 20g–25g pasteurised egg white
...
![](images/framboisier.jpg){fig-alt="An image of Paul Hollywood's Framboisier from Season 13 of Bake Off. The cake is made of two layers of génoise sponge with a layer of crème mousseline on the inside. A layer of raspberries separates the two layers of cake. On the top of the cake there sits a clear sugar dome with a pink (candy) flower on the inside."}
...
## `across()` is easier to understand...

...but `across()` uses a `for()` loop which is computationally intensive!
...
::: columns
::: {.column width="75%"}
The functions in the `purrr` package iterate over the elements of `.x`, but
don't use a `for()` loop at the wrapper level. 

::: {.fragment}
Instead, the looping happens inside an internal helper function (`map_()`), 
...
::: {.column width="20%"}
![](images/purrr.png){fig-alt="The image is a hexagonal logo for the R package purrr. It features a simple, hand-drawn outline of a content, sleeping cat with closed eyes, representing the package's name, which sounds like a cat's purr. The logo's design conveys a sense of calm and ease, suggesting that the purrr package makes functional programming in R more intuitive and manageable."}
:::
:::

::: {.fragment}
So, the loops run **much** faster than loops written in R. 


::: {.callout-note}
# The use of C++ code is not unique to `purrr`!

The `dplyr`, `tidyr`, `data.table`, and `rstan` packages also use C code. 
:::
:::

## `map()`

The `map()` function **iterates** through each item in a list and applies a
