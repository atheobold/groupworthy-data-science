...
<!-- where to use pick() and where to use across() -->
...
<!-- # `group_by()` that takes a tidy-selection of columns to group on. -->

<!-- https://r4ds.hadley.nz/iteration.html#across-in-functions -->

<!-- - := for mutate() in functions -->
<!-- := "name injection": https://dplyr.tidyverse.org/articles/programming.html#name-injection -->
...
This week we're going to take the skills for writing **vector** functions we
learned last week and consider how we might write functions that operate on 
**data frames**.

By the end of the week you should have a grasp of:

<!-- add learning outcomes about using [[]] versus {{ }} to grab variables -->

<!-- add learning outcome about diffusion? -->
...
<!-- makes a common task easier at the cost of making a less common task harder -->

### 📖 [Required Reading: *R4DS* -- Functions](https://r4ds.hadley.nz/functions#data-frame-functions)

::: callout-caution
# Read only Section 3 -- Data Frame Functions!
:::

<!-- Article on the difference between data masking and tidy evaluation: https://dplyr.tidyverse.org/articles/programming.html -->
...
-   this [video by Jenny Bryan](https://youtu.be/2BXPLnLMTYo?si=ry4tHOD77DWagvxn)
    -   I do want to note that this video is from 2019 and some things have changed since then.
    Namely, we used to need to use the `enquo()` function to inject variable names into `dplyr` 
    functions, whereas we now use embracing `{{}}`. 🤗
...
<!-- - what it means to be data masking -->
<!-- - what it means to use tidy evaluation -->
